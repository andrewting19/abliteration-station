...
def tool_output_fingerprint(message: dict) -> str:
    """Hash ordered tool names and exact arguments, without retaining private text."""
    functions = [
        {key: (call.get("function") or {}).get(key, "")
         for key in ("name", "arguments")}
        for call in message.get("tool_calls") or []
    ]
    encoded = json.dumps(functions, sort_keys=True, ensure_ascii=False,
                         separators=(",", ":")).encode()
        ...
        json.dumps(
            {
                ...
                "tool_output_sha256": tool_output_fingerprint(message),
