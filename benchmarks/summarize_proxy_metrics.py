...
def summarize(records: list[dict[str, Any]], hourly_rate: float) -> dict[str, Any]:
    successful = [item for item in records if item.get("status") == 200 and not item.get("error") and not item.get("cancelled")]
    ...
    successful_output_tokens = sum(int((item.get("usage") or {}).get("completion_tokens") or 0) for item in successful)
    ...
    return {
        ...
        "prompt_tps_token_weighted": weighted_rate(successful, "prompt_n", "prompt_per_second"),
        "decode_tps_token_weighted": weighted_rate(successful, "predicted_n", "predicted_per_second"),
        ...
        "usd_per_million_successful_output_tokens": (
            measured_seconds * hourly_rate / 3600 * 1_000_000 / successful_output_tokens
            if successful_output_tokens else None
        ),
